...
export class UserService {
  ...
  private userReposityory: Repository<User>;
  ...
  async register(user: RegisterUserDto) {
    ...
    const foundUser = await this.userReposityory.findOneBy({
    ...
    try {
      await this.userReposityory.save(newUser);
      return '注册成功';
    } catch (error) {
  ...
  //   await this.userReposityory.save([user1, user2]);
  // }
  ...
  async updatePassword(passwordDto: UpdateUserPasswordDto) {
    ...
    try {
      await this.userReposityory.save(foundUser);
      return '密码修改成功';
    } catch (e) {
  ...
  async freezeUserById(id: number) {
    const user = await this.userReposityory.findOneBy({
      id,
    });

    user.isFrozen = true;

    await this.userReposityory.save(user);
  }
  ...
  async findUserByPage(
    ...
    const [users, totalCount] = await this.userReposityory.findAndCount({
      select: [
        'id',
        'username',
        'nickName',
