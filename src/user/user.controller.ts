...
export class UserController {
  ...
  @ApiQuery({
    name: 'refreshToken',
    type: String,
    description: '刷新taoken',
    required: true,
    example: 'xxxxxxxxxxyyyyyyyyyyyzzzzzzzzz',
  })
  @ApiResponse({
  ...
  async googleAuthRedirect(@Req() req, @Res() res: Response) {
    ...
    res.redirect('http://39.108.118.4:82');
  }
  ...
  async githubAuthRedirect(@Req() req, @Res() res: Response) {
    ...
    if (foundUser) {
      ...
      const user = await this.userService.registerByGithubInfo(
        req.user.email,
        req.user.username,
        req.user.picture,
      );
  ...
  async updatePasswrod(@Body() passwordDto: UpdateUserPasswordDto) {
